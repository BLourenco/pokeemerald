...
void CreateBoxMon(struct BoxPokemon *boxMon, u16 species, u8 level, u8 fixedIV, u8 hasFixedPersonality, u32 fixedPersonality, u8 otIdType, u32 fixedOtId)
{
    ...
    if (gBaseStats[species].abilities[1] || gBaseStats[species].abilities[2]) // Has more than 1 ability
    {
        if (gBaseStats[species].abilities[1] && gBaseStats[species].abilities[2]) // Has all 3 abilities
        {
            value = (personality & 0x111) % 3; // 3/8 for Ability 1, 3/8 for Ability 2, 2/8 for Hidden Ability
        }
        else if (!gBaseStats[species].abilities[2]) // Abilities 1 & 2, no hidden
        {
            value = personality & 1;
        }
...
void CalculateMonStats(struct Pokemon *mon)
{
    ...
    else
    {
        if (currentHP == 0 && oldMaxHP == 0)
            currentHP = newMaxHP;
        else if (currentHP != 0 && oldMaxHP != 0) // oldMaxHP is 0 for BoxMon, this prevents the double current HP issue
        {
...
void BoxMonToMon(const struct BoxPokemon *src, struct Pokemon *dest)
{
    u32 value = 0;
    dest->box = *src;
    // Don't heal HP, Status when depositing into PC due to Box Link
