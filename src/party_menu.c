...
static void LoadPartyMenuPCGfx(void);
...
static u8 CreatePCButtonSprite(u8, u8);
...
static void MoveCursorToSlot(u8);
...
static bool8 IsPokemonBoxLinkAvailable(void);
...
static void UsePokemonBoxLink(u8);
...
static void InitPartyMenu(u8 menuType, u8 layout, u8 partyAction, bool8 keepCursorPos, u8 messageId, TaskFunc task, MainCallback callback)
{
    ...
    else
    {
        ...
        sPartyMenuInternal->lastSelectedSlot = PARTY_MENU_SLOT_MON_1;
        ...
        if (!keepCursorPos)
            gPartyMenu.slotId = PARTY_MENU_SLOT_MON_1;
    ...
    else
    {
        ...
        else
        {
            if (IsPokemonBoxLinkAvailable())
                sPartyMenuInternal->spriteIdCancelPokeball = CreatePCButtonSprite(198, 148);
            else
...
void AnimatePartySlot(u8 slot, u8 animNum)
{
    ...
    default:
        ...
        return;
    case PARTY_MENU_SLOT_CONFIRM_BUTTON:
        if (animNum == 0)
        ...
        break;
    case PARTY_MENU_SLOT_CANCEL_BUTTON:
...
static u8 GetPartyBoxPaletteFlags(u8 slot, u8 animNum)
{
    ...
    if (gPartyMenu.action == PARTY_ACTION_SWITCHING_MONS)
        palFlags |= PARTY_PAL_SWITCHING;
    if (gPartyMenu.action == PARTY_ACTION_SWITCH_MONS || gPartyMenu.action == PARTY_ACTION_SWITCH_ITEMS)
    {
...
static bool8 PartyBoxPal_ParnterOrDisqualifiedInArena(u8 slot)
{
    if (gPartyMenu.layout == PARTY_LAYOUT_MULTI 
        && (slot == PARTY_MENU_SLOT_MON_2
         || slot == PARTY_MENU_SLOT_MON_4
         || slot == PARTY_MENU_SLOT_MON_6))
        return TRUE;
...
void Task_HandleChooseMonInput(u8 taskId)
{
    if (!gPaletteFade.active && MenuHelpers_CallLinkSomething() != TRUE)
    {
        ...
        switch (PartyMenuButtonHandler(slotPtr)) // Check all buttons
        {
        ...
        case B_BUTTON: // Selected Cancel
            ...
            break;
        case R_BUTTON:
            MoveCursorToSlot(PARTY_MENU_SLOT_PC_BUTTON);
            UsePokemonBoxLink(taskId);
            break;
        case START_BUTTON: // Start button
            if (sPartyMenuInternal->chooseHalf)
            {
                PlaySE(SE_SELECT);
                MoveCursorToSlot(PARTY_MENU_SLOT_CONFIRM_BUTTON);
            }
            else if (GetPartyMenuType() == PARTY_MENU_TYPE_FIELD 
                    && GetMonData(&gPlayerParty[1], MON_DATA_SPECIES) != SPECIES_NONE)
            {
                switch (gPartyMenu.action)
                {
                    case PARTY_ACTION_CHOOSE_MON:
                        sCursorOptions[MENU_MOVE_ITEM].func(taskId);
                        break;
                    default:
                        HandleChooseMonSelection(taskId, slotPtr);
                        break;
            ...
            break;
        case SELECT_BUTTON:
            if (GetPartyMenuType() == PARTY_MENU_TYPE_FIELD 
                && GetMonData(&gPlayerParty[1], MON_DATA_SPECIES) != SPECIES_NONE)
            {
                switch (gPartyMenu.action)
                {
                    case PARTY_ACTION_CHOOSE_MON:
                        sCursorOptions[MENU_SWITCH].func(taskId);
                        break;
                    default:
                        HandleChooseMonSelection(taskId, slotPtr);
                        break;
...
static s8* GetCurrentPartySlotPtr(void)
{
    if (gPartyMenu.action == PARTY_ACTION_SWITCH_MONS || gPartyMenu.action == PARTY_ACTION_SWITCH_ITEMS || gPartyMenu.action == PARTY_ACTION_SOFTBOILED)
...
{
    if (*slotPtr == PARTY_MENU_SLOT_CONFIRM_BUTTON)
    {
    ...
    else
    {
        ...
        case PARTY_ACTION_GIVE_PC_ITEM:
            ...
            break;
        case PARTY_ACTION_SWITCH_MONS:
            PlaySE(SE_SELECT);
            SwitchSelectedMons(taskId);
            break;
        case PARTY_ACTION_SWITCH_ITEMS:
        ...
        case PARTY_ACTION_SWITCHING_MONS:
            PlaySE(SE_SELECT);
...
static void HandleChooseMonCancel(u8 taskId, s8 *slotPtr)
{
    ...
    default:
        ...
        {
            if (!MenuHelpers_LinkSomething())
                gSpecialVar_0x8004 = PARTY_MENU_SLOT_CANCEL_BUTTON;
...
static void Task_HandleCancelChooseMonYesNoInput(u8 taskId)
{
    ...
    case 0:
        gPartyMenuUseExitCallback = FALSE;
        gPartyMenu.slotId = PARTY_MENU_SLOT_CANCEL_BUTTON;
...
static u16 PartyMenuButtonHandler(s8 *slotPtr)
{
    ...
    if (JOY_NEW(R_BUTTON) && IsPokemonBoxLinkAvailable())
        return R_BUTTON;
...
static void UpdatePartySelectionSingleLayout(s8 *slotPtr, s8 movementDir)
{
    switch (movementDir)
    {
        case MENU_DIR_UP:
            switch (*slotPtr)
            {
                case PARTY_MENU_SLOT_MON_1:
                case PARTY_MENU_SLOT_MON_2:
                    SetSlotPtr(slotPtr, PARTY_MENU_SLOT_CANCEL_BUTTON);
                    break;
                case PARTY_MENU_SLOT_CONFIRM_BUTTON:
                ...
                case PARTY_MENU_SLOT_CANCEL_BUTTON:
                    if (sPartyMenuInternal->chooseHalf)
                        SetSlotPtr(slotPtr, PARTY_MENU_SLOT_CONFIRM_BUTTON);
                    else
                        SetSlotPtr(slotPtr, GetLastMonSlotInLastColumnSingleLayout());
                    break;
                default:
            ...
            switch (*slotPtr)
            {
                case PARTY_MENU_SLOT_CANCEL_BUTTON:
                    if (sPartyMenuInternal->lastSelectedSlot % 2 == 0) // Slots 0, 2, 4 (Left side)
                        SetSlotPtr(slotPtr, PARTY_MENU_SLOT_MON_1);
                    else
                        SetSlotPtr(slotPtr, PARTY_MENU_SLOT_MON_2);
                    break;
                case PARTY_MENU_SLOT_CONFIRM_BUTTON:
                    SetSlotPtr(slotPtr, PARTY_MENU_SLOT_CANCEL_BUTTON);
                    break;
...
static void UpdatePartySelectionDoubleLayout(s8 *slotPtr, s8 movementDir)
{
    ...
    case MENU_DIR_UP:
        ...
        else if (*slotPtr == PARTY_MENU_SLOT_CONFIRM_BUTTON)
        {
            *slotPtr = gPlayerPartyCount - 1;
            break;
        }
        else if (*slotPtr == PARTY_MENU_SLOT_CANCEL_BUTTON)
        {
            if (sPartyMenuInternal->chooseHalf)
            {
                *slotPtr = PARTY_MENU_SLOT_CONFIRM_BUTTON;
                break;
            }
            (*slotPtr)--;
        }
...
static s8 GetNewSlotDoubleLayout(s8 slotId, s8 movementDir)
{
    ...
    {
        slotId += movementDir;
        if ((u8)slotId == PARTY_MENU_SLOT_CONFIRM_BUTTON || (u8)slotId == PARTY_MENU_SLOT_CANCEL_BUTTON)
            return -1;
...
static void SetSlotPtr(s8 *slotPtr, s8 slotId)
{
    ...
    if ((slotId < PARTY_SIZE && GetMonData(&gPlayerParty[slotId], MON_DATA_SPECIES) == SPECIES_NONE)
        || (slotId == PARTY_MENU_SLOT_CONFIRM_BUTTON && !sPartyMenuInternal->chooseHalf))
    {
        ...
        {
            if (sPartyMenuInternal->chooseHalf)
                newSlotId = PARTY_MENU_SLOT_CONFIRM_BUTTON;
            else
                newSlotId = PARTY_MENU_SLOT_CANCEL_BUTTON;
        }
        
        *slotPtr = slotId;
        SetSlotPtr(slotPtr, newSlotId); // Try again in the next slot
    }
    else if (slotId < PARTY_MENU_SLOT_MON_1)
    {
        SetSlotPtr(slotPtr, PARTY_MENU_SLOT_CANCEL_BUTTON);
    }
    else
...
static s8 GetLastMonSlotInLastColumnSingleLayout()
{
    ...
    {
        case PARTY_MENU_SLOT_MON_1:
        case PARTY_MENU_SLOT_MON_3:
        case PARTY_MENU_SLOT_MON_5:
            if (GetMonData(&gPlayerParty[PARTY_MENU_SLOT_MON_5], MON_DATA_SPECIES) != SPECIES_NONE)
                return PARTY_MENU_SLOT_MON_5;
            else if (GetMonData(&gPlayerParty[PARTY_MENU_SLOT_MON_3], MON_DATA_SPECIES) != SPECIES_NONE)
                return PARTY_MENU_SLOT_MON_3;
            else
                return PARTY_MENU_SLOT_MON_1;
            break;
        case PARTY_MENU_SLOT_MON_2:
        case PARTY_MENU_SLOT_MON_4:
        case PARTY_MENU_SLOT_MON_6:
            if (GetMonData(&gPlayerParty[PARTY_MENU_SLOT_MON_6], MON_DATA_SPECIES) != SPECIES_NONE)
                return PARTY_MENU_SLOT_MON_6;
            else if (GetMonData(&gPlayerParty[PARTY_MENU_SLOT_MON_4], MON_DATA_SPECIES) != SPECIES_NONE)
                return PARTY_MENU_SLOT_MON_4;
            else if (GetMonData(&gPlayerParty[PARTY_MENU_SLOT_MON_2], MON_DATA_SPECIES) != SPECIES_NONE)
                return PARTY_MENU_SLOT_MON_2;
            else
                return PARTY_MENU_SLOT_MON_1;
            break;
        default:
...
static bool8 IsPokemonBoxLinkAvailable(void)
{
    return gPartyMenu.menuType == PARTY_MENU_TYPE_FIELD
            && !(gMapHeader.flags & MAP_ALLOW_ESCAPING)
            && !FlagGet(FLAG_SYS_FORCE_POKEMON_BOX_LINK_UNAVAILABLE);
}
...
static void CreateCancelConfirmWindows(bool8 chooseHalf)
{
    ...
    if (gPartyMenu.menuType != PARTY_MENU_TYPE_MULTI_SHOWCASE)
    {
        ...
        if (IsPokemonBoxLinkAvailable())
        {
            mainOffset = GetStringCenterAlignXOffset(0, gText_PC, 48);
            AddTextPrinterParameterized3(cancelWindowId, 0, mainOffset + offset, 1, sFontColorTable[0], -1, gText_PC);
        }
        else
        {
...
static void DisplayPartyPokemonLevelCheck(struct Pokemon *mon, struct PartyMenuBox *menuBox, u8 c)
{
    if (GetMonData(mon, MON_DATA_SPECIES) != SPECIES_NONE)
    {
        // Hide level when Mon has a Status
...
static bool8 CreateSelectionWindow(u8 taskId)
{
    ...
    else
    {
        ...
        {
            StringExpandPlaceholders(gStringVar4, gText_PkmnNotHoldingItem);
            DisplayPartyMenuMessage(gStringVar4, TRUE);
            ScheduleBgCopyTilemapToVram(2);
...
static void CursorCb_Nickname(u8 taskId)
{
    ...
    sPartyMenuInternal->exitCallback = ChangePartyPokemonNickname;
    Task_ClosePartyMenu(taskId);
}
...
{
    PlaySE(SE_SELECT);
    gPartyMenu.action = PARTY_ACTION_SWITCH_MONS;
    PartyMenuRemoveWindow(&sPartyMenuInternal->windowId[1]);
    PartyMenuRemoveWindow(&sPartyMenuInternal->windowId[0]);
...
static void SwitchSelectedMons(u8 taskId)
{
    ...
    else
    {
        ...
        gPartyMenu.action = PARTY_ACTION_SWITCHING_MONS;
        AnimatePartySlot(gPartyMenu.slotId, 1);
...
static void Task_UpdateHeldItemSprite(u8 taskId)
{
    ...
    if (IsPartyMenuTextPrinterActive() != TRUE)
    {
        ...
        if (gPartyMenu.action == PARTY_ACTION_SWITCH_ITEMS)
            FinishTwoMonAction(taskId);
        else            
            Task_ReturnToChooseMonAfterText(taskId);
    }
...
static void SwitchSelectedMonsItems(u8 taskId)
{
    ...
    else if (GetMonData(&gPlayerParty[gPartyMenu.slotId2], MON_DATA_IS_EGG))
    {
        PlaySE(SE_FAILURE);
    }
    else
...
void CursorCb_MoveItem(u8 taskId)
{
    ...
    {
        PlaySE(SE_SELECT);
        gPartyMenu.action = PARTY_ACTION_SWITCH_ITEMS;
    ...
    else
    {
        ...
        if (GetMonData(mon, MON_DATA_IS_EGG))
        {
            StringCopy(gStringVar4, gText_EggsCantHoldItems);
            DisplayPartyMenuMessage(gStringVar4, TRUE);
        }
        else
        {
...
static void MoveCursorToSlot(u8 slotId)
{
    AnimatePartySlot(gPartyMenu.slotId, 0);
...
static u8 CreatePCButtonSprite(u8 x, u8 y)
{
    u8 spriteId = CreateSprite(&sSpriteTemplate_MenuPC, x, y, 8);
...
static void LoadPartyMenuPCGfx(void)
{
    LoadCompressedSpriteSheet(&sSpriteSheet_MenuPC);
    LoadCompressedSpritePalette(&sSpritePalette_MenuPC);
}
...
void CB2_ShowPartyMenuForItemUse(void)
{
    ...
    {
        menuType = PARTY_MENU_TYPE_FIELD_NO_PC_ACCESS;
...
void ItemUseCB_Medicine(u8 taskId, TaskFunc task)
{
    ...
    if (cannotUse != FALSE)
    {
        ...
        ScheduleBgCopyTilemapToVram(2);
        if (gPartyMenu.menuType == PARTY_MENU_TYPE_FIELD_NO_PC_ACCESS)
            gTasks[taskId].func = Task_ReturnToChooseMonAfterText;
        else
    ...
    else
    {
        ...
        else
        {
            ...
            DisplayPartyMenuMessage(gStringVar4, TRUE);
            ScheduleBgCopyTilemapToVram(2);
            if (gPartyMenu.menuType == PARTY_MENU_TYPE_FIELD_NO_PC_ACCESS && CheckBagHasItem(item, 1))
                gTasks[taskId].func = Task_ReturnToChooseMonAfterText;
            else
                gTasks[taskId].func = task;
        }
...
static void Task_DisplayHPRestoredMessage(u8 taskId)
{
    ...
    if (gPartyMenu.menuType == PARTY_MENU_TYPE_FIELD_NO_PC_ACCESS && CheckBagHasItem(gSpecialVar_ItemId, 1))
        gTasks[taskId].func = Task_ReturnToChooseMonAfterText;
    else
        gTasks[taskId].func = Task_ClosePartyMenuAfterText;
}
...
void CB2_ChooseMonToGiveItem(void)
{
    ...
    InitPartyMenu(PARTY_MENU_TYPE_FIELD_NO_PC_ACCESS, PARTY_LAYOUT_SINGLE, PARTY_ACTION_GIVE_ITEM, FALSE, PARTY_MSG_GIVE_TO_WHICH_MON, Task_HandleChooseMonInput, callback);
...
{
    InitPartyMenu(PARTY_MENU_TYPE_FIELD_NO_PC_ACCESS, PARTY_LAYOUT_SINGLE, PARTY_ACTION_GIVE_MAILBOX_MAIL, FALSE, PARTY_MSG_GIVE_TO_WHICH_MON, Task_HandleChooseMonInput, Mailbox_ReturnToMailListAfterDeposit);
}
...
{
    InitPartyMenu(PARTY_MENU_TYPE_FIELD_NO_PC_ACCESS, PARTY_LAYOUT_SINGLE, PARTY_ACTION_MOVE_TUTOR, FALSE, PARTY_MSG_TEACH_WHICH_MON, Task_HandleChooseMonInput, CB2_ReturnToFieldContinueScriptPlayMapMusic);
}
...
static void BufferBattlePartyOrder(u8 *partyBattleOrder, u8 flankId)
{
    ...
    if (IsMultiBattle() == TRUE)
    {
        // When player and partner mons are loaded, the player's are loaded into 
        // the first 3 slots, and the partner's into the last 3 slots
        // partyBattleOrder has 3 bytes, one per index
        // Each byte holds the positions of 2 Pokemon, each using 4 bits
        // The first byte are the lead Mons, the last two bytes are
        // the rest.
        if (flankId != 0)
        {
            partyBattleOrder[0] = (PARTY_MENU_SLOT_MON_4 << 4) | PARTY_MENU_SLOT_MON_1;
            partyBattleOrder[1] = (PARTY_MENU_SLOT_MON_5 << 4) | PARTY_MENU_SLOT_MON_2;
            partyBattleOrder[2] = (PARTY_MENU_SLOT_MON_6 << 4) | PARTY_MENU_SLOT_MON_3;
        }
        else
        {
            partyBattleOrder[0] = (PARTY_MENU_SLOT_MON_1 << 4) | PARTY_MENU_SLOT_MON_4; // Slot 1 Mon (player's first) and Slot 4 Mon (Partner's first) are the leads, put them in the first two slots
            partyBattleOrder[1] = (PARTY_MENU_SLOT_MON_2 << 4) | PARTY_MENU_SLOT_MON_5;
            partyBattleOrder[2] = (PARTY_MENU_SLOT_MON_3 << 4) | PARTY_MENU_SLOT_MON_6;
        }
        return;
    }
...
static void BufferBattlePartyOrderBySide(u8 *partyBattleOrder, u8 flankId, u8 battlerId)
{
    ...
    if (IsMultiBattle() == TRUE)
    {
        ...
        {
            partyBattleOrder[0] = (PARTY_MENU_SLOT_MON_1 << 4) | PARTY_MENU_SLOT_MON_4;
            partyBattleOrder[1] = (PARTY_MENU_SLOT_MON_2 << 4) | PARTY_MENU_SLOT_MON_5;
            partyBattleOrder[2] = (PARTY_MENU_SLOT_MON_3 << 4) | PARTY_MENU_SLOT_MON_6;
        }
        return;
...
static void UsePokemonBoxLink(u8 taskId)
{
    PlaySE(SE_SELECT);
...
{
    gPaletteFade.bufferTransferDisabled = TRUE;
    gPartyMenu.slotId = PARTY_MENU_SLOT_PC_BUTTON;
    FlagClear(FLAG_SYS_REMOTE_PC_ACTIVE);
