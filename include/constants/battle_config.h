...
#define B_EXP_GAIN_FORMULA          GEN_8 // In Gen5 and Gen7+, EXP gain is scaled based on the opponent Pokemon's Level and your Pokemon's Level.
...
// Ability settings
#define B_ABILITY_WEATHER           GEN_8 // In Gen5+, weather caused by abilities lasts the same amount of turns as induced from a move. Before, they lasted till the battle's end or weather change by a move.
#define B_GALE_WINGS                GEN_8 // In Gen7+ requires full HP to trigger.
#define B_STANCE_CHANGE_FAIL        GEN_8 // In Gen7+, Aegislash's form change does not happen, if the Pokémon cannot use a move, because of confusion, paralysis, etc. In gen6, the form change occurs despite not being able to move.
